...
public class SwaggerUiPlaywrightTests : PageTest
{
            ...
            .ConfigureWebHost(webBuilder =>
            {
                ...
                webBuilder.ConfigureServices(services =>
                {
                    ...
                    services.AddSignalROpenApi(options =>
                    {
                        ...
                        // Only surface this test app's own hubs, so the rendered UI is not
                        // affected by hubs belonging to other test classes.
                        options.HubFilter = t =>
                            t == typeof(BasicHub)
                            || t == typeof(StreamingHub)
                            || t == typeof(TypedChatHub)
                            || t == typeof(UiExampleHub);
                    });
                ...
                webBuilder.Configure(app =>
                {
                    ...
                    app.UseEndpoints(endpoints =>
                    {
                        ...
                        endpoints.MapHub<UiExampleHub>("/hubs/uiexample");
    ...
    public async Task SwaggerUi_ResponseExamplesDisplayed()
    {
        ...
        // The responses section contains more than one <select>: SwaggerUI renders a
        // media-type dropdown ("application/json") alongside the named-examples dropdown.
        // Match on the option text so the examples dropdown is found regardless of order.
        var exampleSelect = responsesSection.Locator("select")
            .Filter(new() { HasTextString = "Created" });

        var sectionContent = await responsesSection.InnerTextAsync();
        Assert.IsTrue(
            await exampleSelect.CountAsync() > 0,
            $"The named-examples dropdown should be rendered. Section content: {sectionContent}");

        var selectHtml = await exampleSelect.First.InnerHTMLAsync();
        Assert.IsTrue(
            selectHtml.Contains("Created") && selectHtml.Contains("Pending"),
            $"Example dropdown should list both provided examples. HTML: {selectHtml}");

        // The selected example's value must actually be rendered, not just named.
        Assert.IsTrue(
            sectionContent.Contains("ORD-001"),
            $"Response section should show the selected example's value. Content: {sectionContent}");
    }
