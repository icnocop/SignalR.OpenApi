// Copyright (c) SignalR.OpenApi Contributors. Licensed under the MIT License.

using SignalR.OpenApi.Examples;

namespace SignalR.OpenApi.Tests.Generation.GenerateDocument.Examples.DependencyInjection;

/// <summary>
/// An examples provider that requires a dependency injected via the constructor.
/// </summary>
public class DiRequestExamplesProvider : ISignalROpenApiExamplesProvider<DiRequest>
{
    private readonly ITestValueProvider valueProvider;

    /// <summary>
    /// Initializes a new instance of the <see cref="DiRequestExamplesProvider"/> class.
    /// </summary>
    /// <param name="valueProvider">The injected value provider.</param>
    public DiRequestExamplesProvider(ITestValueProvider valueProvider)
    {
        this.valueProvider = valueProvider;
    }

    /// <inheritdoc/>
    public IEnumerable<SignalROpenApiExample<DiRequest>> GetExamples()
    {
        yield return new SignalROpenApiExample<DiRequest>(
            "DiExample",
            new DiRequest { Name = this.valueProvider.GetValue() })
        {
            Summary = "Example from DI provider",
        };
    }
}
